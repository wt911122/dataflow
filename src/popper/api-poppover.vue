<template>
    <div :class="$style.root">
        <div :class="$style.value">URL：<input :value="url" @change="onChangeURL" /></div>
        <div>Response:<textarea rows="4" cols="50" readonly>{{ nodeValue }}</textarea></div>
    </div>
</template>
<script setup>
...
const url = ref(props.source.signal.url)
