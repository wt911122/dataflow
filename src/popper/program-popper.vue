<template>
    <div :class="$style.root">
        <div>输入：{{ input }}</div>
        <div>输出：<textarea rows="4" cols="50" readonly>{{ output }}</textarea></div>
...
// const operator = computed(() => props.source.operator);
const input = ref('');
...
const onChange = (e) => {
    ...
    const operator = toRaw(props.source.operator);
...
const updateValue = () => {
    const operator = toRaw(props.source.operator);
    let inputContent = [];
    for(let key in operator.input) {
        const u = operator.input[key];
        inputContent.push(`${key}=${u}`);
    }
    output.value = operator.output.getValue();
